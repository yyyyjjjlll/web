...
>
> AI 流式输出就是这个道理 —— 不是等全部内容生成完再给你看，而是一边生成一边显示给你看。
...
【第一步】用户发送消息
            ↓
【第二步】前端 → 后端：告诉我你要开始生成AI回复了
            ↓
【第三步】后端 → AI服务器：发送请求（注意：stream: true）
            ↓
【第四步】AI服务器 → 后端：一块一块地返回内容（流式的）
            ↓
【第五步】后端 → 前端：通过IPC通道一块一块推送给你
            ↓
【第六步】前端：收到一块，显示一块到屏幕上
            ↓
【第七步】全部接收完成，前端解析并保存到数据库
```
...
async *generateStream(...): AsyncGenerator<string> {
  ...
  for await (const chunk of stream) {
    ...
    if (content) {
      yield content  // 把这块内容"产出"出去，给下一棒
    }
  }
}
```
...
{isLoading && (
  <div>
    {streamingContent ? (
      ...
      <p>
        {streamingContent}
        <span className="animate-pulse">|</span>  {/* 光标在闪烁 */}
      </p>
    ) : (
      // 没内容时：显示等待动画
...
> 💡 **新手必看：什么是 Zustand？为什么要用它？**

想象一下如果没有 Zustand：
- 聊天记录存在哪里？组件A发送的消息，组件B怎么看到？
- 每次切换房间，都要传很多数据，好麻烦！
- 多个地方都要用到"当前房间ID"，怎么同步？
...
```typescript
// src/renderer/src/store/appStore.ts（这是我们的"仓库"）
...
interface AppState {
  rooms: Room[]           // 所有房间列表
  characters: Character[] // 当前房间的角色列表
  messages: Message[]     // 当前房间的消息列表
  currentRoomId: string | null  // 当前正在看的房间
...
> 📝 **Zustand 的好处**
> 1. **简单**：比 Redux 简单太多了，不需要写各种 reducer
> 2. **TypeScript 友好**：类型推断很智能
> 3. **不需要 Provider**：不需要像 Redux 那样包一层 Provider
> 4. **灵活**：可以在任何地方调用，组件内外都可以
